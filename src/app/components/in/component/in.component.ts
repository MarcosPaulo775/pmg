...
export class InComponent implements OnInit {
  ...
  updateTable() {
    this.apiService.custom_objects_list('chapa', ['deleted', 'equal to', false, 'and', 'status', 'equal to', 'in'], ' ')
  ...
  onOut(id){
    this.apiService.custom_objects_set_keys('chapa', id, { 'status': 'out' })
      .subscribe((data: Chapa) => {
        if (!data.error) {
          this.appService.openSnackBar('Excluido', 'ok');
          this.updateTable();
